...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME)
      ...
      .catch(() => {}) // Don't fail install if caching fails (e.g. in microfrontend dev mode)
  );
  self.skipWaiting();
});
...
// ── Background push (from Edge Function via cron) ────────────────────────────
self.addEventListener('push', (event) => {
  if (!event.data) return;
  let data;
  try { data = event.data.json(); } catch { data = { title: 'Finance Tracker', body: event.data.text() }; }
  event.waitUntil(
